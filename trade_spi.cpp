...
// #if _IS_LOG_SHOW_
	...
	if (error_info)
	{
		cout << "Order Error:" << error_info->error_id << endl;
	}
// #endif
	updateOrderNum();
...
// #if _IS_LOG_SHOW_
	...
	cout << "Trade_id:" << trade_info->exec_id << endl;
	cout << "Instrumentid:" << trade_info->ticker<< endl;
...
void MyTraderSpi::OnQueryPosition(XTPQueryStkPositionRsp * investor_position, XTPRI * error_info, int request_id, bool is_last, uint64_t session_id)
{
	if (error_info && error_info->error_id !=0)
	{
		...
		{
			//cout<<"instid="<<investor_position->ticker<<", total_qty="<<investor_position->total_qty<<", sellable_qty="<<investor_position->sellable_qty<<", avg_price="<<investor_position->avg_price<<endl;
		}
		return;
	}
	cout<<"instid="<<investor_position->ticker<<", total_qty="<<investor_position->total_qty<<", sellable_qty="<<investor_position->sellable_qty<<", avg_price="<<investor_position->avg_price<<endl;

	string init_num = "set a1_" + string(investor_position->ticker) + "_sellable_qty " + std::to_string(investor_position->sellable_qty);
	(redisReply*)redisCommand(my_redis, init_num.c_str());
	string pp_num = "set a1_" + string(investor_position->ticker) + "_avg_price " + std::to_string(investor_position->sellable_qty);
	(redisReply*)redisCommand(my_redis, pp_num.c_str());
// 	if (save_to_file_)
// 	{
// #ifdef WIN32
// 		SYSTEMTIME wtm;
